...
uint8_t* MIDI_Start()
{
	...
	// Initialize RX Pins (USART1 TXPB7)
	...
	// Connect the RX  pins to USART1 alternative function
		...
		switch (state)
		{
			...
			case 11 :
			{
				...
				if (MIDI_velocity > 0)
				{
					...
					if(!note_exists(note_list, MIDI_note))
					{
						note_list = add_note_last(note_list, MIDI_note, MIDI_velocity);
					}else{

						change_note_state(note_list, MIDI_note,1);
					}
				...
				else
				{
					...
					change_note_state(note_list, MIDI_note,4);
				}
			...
			case 31 :
			{
				...
				state = 30;														// Next state is 30
				if (MIDI_CC_value == 127)
					params.pedal=1;
				else
					params.pedal=0;
				//printf ("CC : %d %d\n", MIDI_CC_number, MIDI_CC_value);
			//	ChangeParam(MIDI_CC_number, MIDI_CC_value);
			//	params.bend = (float_t)MIDI_CC_value ;
			//	if(MIDI_CC_number == 64 )
				//	params.detune = 1.0f + 0.0004f*(MIDI_CC_value-64);
				break;
			}

...
int main(void)
{
	...
	uint16_t _btn_count = 0;
	uint8_t*	pMIDI_buffer;		// MIDI variables
	uint8_t		nb_MIDI_bytes;
	uint8_t*	pPARAMETERS_buffer;		// PARAMETERS variables
	uint8_t		nb_PARAMETERS_bytes;
	...
	pPARAMETERS_buffer = PARAMETERS_Start();
	...
	params.pitch = 220.0f;
		...
		params.osc2_octave = 2.0f;

		params.osc1_mix = 0.75f;
		params.osc2_mix = 0.75f;

		params.cutoff = 24.0f;
		params.reso = 1.0f;

		params.adsr1_attack = 0.01f;
		params.adsr1_decay = 0.01f;
		params.adsr1_sustain = 0.5f;
		params.adsr1_release = 0.01f;
		...
		params.lfo1_depth = 0.5f;


		params.lfo2_frequency = 1.0f;
		params.lfo2_depth = 0.0f;
    ...
    while(1)
    {
    	...
    	nb_PARAMETERS_bytes = PARAMETERS_GetNbNewBytes(pPARAMETERS_buffer);

		// If yes, parse message

		if (nb_PARAMETERS_bytes)
		{
		//	printf(" parameters bytes arrived : %d\n", nb_PARAMETERS_bytes);
			PARAMETERS_parser(pPARAMETERS_buffer, nb_PARAMETERS_bytes);
		}
		if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0) == 1)
		{
			_btn_count++;
				if (_btn_count > 0xFFFE)
				{
					printf("Button");
					_btn_count = 0;
				}
...
{
	//printf("Here is a USART1 interrupt\n");
}
...
{
	//printf("Here is a DMA 2 interrupt\n");
